...
/**
 * Browser port of `backend/src/modules/pack-generator/strategies/StandardGenerationStrategy.ts`
 * for the Phase 4 provably-fair verifier. The randomness contract is
 * identical: every `Math.random()` call site in the server strategy maps to a
 * `rand()` call here. Pair it with a seeded RNG built by `seededRng.ts` to
 * reproduce a Phase 2 derivation byte-for-byte.
 *
 * Only the config knobs the strategy actually reads are mirrored. They must
 * stay in lockstep with `packGenerator.config.ts`; any change there for
 * `algorithm_version = "standard_v1"` is a breaking change to the verifier.
 */
...
function effectiveRetailSwingProbability(dryStreak: number): number {
...
interface TierWindow {
  anchorMinUsd: Decimal;
  anchorMaxUsd: Decimal;
}
...
export class StandardGenerationStrategy {
  ...
  generateOnePack(
    ...
    if (afterAnchor.greaterThan(0)) {
      ...
      const minStep2 = afterAnchor.mul(
        vetoLowRoll ? Math.max(STEP2_MIN_RATIO, VETO_MIN_RATIO) : STEP2_MIN_RATIO
      );
  ...
  private tryRetailSwingPack(
    ...
    const cap = retailPrice.mul(STANDARD_V1_CONFIG.retailSwingPackValueCapRatio);
    if (candidates.length < 3) return null;

    for (let attempt = 0; attempt < SWING_RANDOM_TRIES; attempt += 1) {
  ...
  private filterByRange(pool: CandidateCard[], min: Decimal, max: Decimal): CandidateCard[] {
    return pool.filter(
      (c) => c.marketValue.greaterThanOrEqualTo(min) && c.marketValue.lessThanOrEqualTo(max)
    );
  }
